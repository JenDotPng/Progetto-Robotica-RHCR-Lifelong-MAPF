...
bool LRAStar::run(const vector<State>& starts, const vector< vector<pair<int, int> > >& goal_locations, int time_limit)
{
	...
	// resolve conflicts
	resolve_conflicts(shortest_paths, starts); 
	runtime = (std::clock() - start) * 1.0 / CLOCKS_PER_SEC;
	return true;
}
...
void LRAStar::resolve_conflicts(const vector<Path>& input_paths, const vector<State>& starts)
{
    num_wait_commands = 0;
    ...
    {
        State start_state = input_paths[k].empty() ? starts[k] : input_paths[k][0];
        solution[k].push_back(start_state);
        if (k_robust == 1)
            curr_locations[start_state.location] = k;
    }
    ...
    for (int t = 1; t <= simulation_window; t++)
    {
        ...
        for (int k = 0; k < num_of_agents; k++) agents_list[k] = k;
        ...
        for (auto agent : agents_list)
        {
            ...
            if (loc == solution[agent][t - 1].location)
            {
                ...
                if (other != next_locations.end())
                {
                    ...
                    path_pointers[other->second] = max(0, path_pointers[other->second] - 1); // Protezione decremento
                }
            }
        ...
        if (k_robust == 1) curr_locations = next_locations;
    }
    print_results();
}


// --- SOVRACCARICO 1: Per traj_pointers ---
void LRAStar::wait_command(int agent, int timestep,
        vector<list<pair<int, int> >::const_iterator >& traj_pointers)
{
    // Limite di sicurezza per evitare stack overflow in caso di deadlock circolare
    if (num_wait_commands > 10000) return; 
    ...
    auto other = next_locations.find(location); 
    if (other != next_locations.end() && other->second != agent)
    {
        int next_agent = other->second;
        next_locations.erase(other); // Rompe il ciclo potenziale
        wait_command(next_agent, timestep, traj_pointers); 
        --traj_pointers[next_agent];
    }
    next_locations[location] = agent;
    num_wait_commands++;
}

// --- SOVRACCARICO 2: Per path_pointers (quello che causava l'errore) ---
void LRAStar::wait_command(int agent, int timestep, vector<int>& path_pointers)
{
    // Protezione contro stack overflow (troppi conflitti ricorsivi)
    if (num_wait_commands > 10000) return;
    ...
    if (other != next_locations.end() && other->second != agent)
    {
        wait_command(other->second, timestep, path_pointers);
        path_pointers[other->second] = max(0, path_pointers[other->second] - 1); // Protezione decremento
    }
    next_locations[location] = agent;
    num_wait_commands++;
