"""
Stress test multi-solver mappa sorting nativa per scenario SORTING.

Scopo:
- eseguire benchmark automatici variando numero agenti, seed e solver;
- raccogliere metriche robuste da solver.csv (runtime medio e throughput);
- produrre report aggregati e grafici finali per confronto sperimentale.

Funzionamento:
1) costruisce i comandi CLI verso lifelong.exe con parametri fissati (mappa, h, w, tempo);
2) esegue i test in triplo ciclo: seed -> numero agenti -> solver;
3) classifica ogni run in Success/Fail/Timeout;
4) pulisce la cache euristica della mappa dopo ogni run per evitare contaminazioni;
5) salva due report CSV:
     - final_report.csv (dati run-level)
     - final_report_summary.csv (aggregazione per solver e numero agenti)
6) genera grafici PNG di runtime e throughput.

Modalita di utilizzo:
- esecuzione completa: python final_stress_test.py
- solo report/grafici da CSV esistente:
    python final_stress_test.py --report-only --output-base <cartella_output>
...
def run_super_test():
    ...
    AGENTS = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200]
    SEEDS = [42]

    TIME = 3000
    CUTOFF = 240
    PROCESS_TIMEOUT = 300
    OUTPUT_BASE = "exp/stress_test_sorting_map/final_comparison"
    MAP = "maps/sorting_map.grid"
    
    solver_cfg = {
        "PBS": {"extra": "", "w": 5, "h": 5},
        "ECBS": {"extra": "--suboptimal_bound 1.5", "w": 10, "h": 5},
        "WHCA": {"extra": "", "w": 10, "h": 5},
    }
